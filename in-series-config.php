...
$basic_config_mode_label = __("Switch to Basic", "in_series");

if($config_mode == 1 || $config_mode === $adv_config_mode_label) {
    $default_adv_config_mode = "0";
    $config_mode_label = $basic_config_mode_label;
    require_once("in-series-config-advanced.php");
}
else {
    $default_adv_config_mode = "1";
    $config_mode_label = $adv_config_mode_label;
...
<div class='wrap in_series_config'>
  ...
  <p><button type='submit' name='in_series[adv_config_mode]' value='{$default_adv_config_mode}'>${config_mode_label}</button></p>
  <p class='submit'>
