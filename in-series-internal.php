...
class InSeriesInternal {
    ...
    /**************************************************************************\
     *            Compatibility Shims for Broken WordPress APIs               *
    \**************************************************************************/
    ...
    /**
     * @return int the pid of the inserted post
     *
     * Post data needs to be escaped before being passed. Do this in one place,
     * instead of all over.
     */
    ...
    function ToCHtml($format = false) {
        ...
        $active_entry_format = wptexturize($active_entry_format);
        $entry_format = wptexturize($entry_format);
        $toc_format = wptexturize($toc_format);
    ...
    function SeriesListHtml($format = false) {
        ...
        $list_entry_format = wptexturize($list_entry_format);
        $list_series_format = wptexturize($list_series_format);
    ...
    function FormatHtml($format, $pid) {
        ...
        $series = wptexturize(stripslashes($series->series_name));
    ...
    function process_save($epid) {
        ...
        if(empty($_POST['in_series']) ||
           (empty($_POST['in_series']['add_to_series']['series']) &&
            ...
            empty($_POST['in_series']['existing']))) {
            $series_delta = array();
...
INSERT INTO {$entry_table} (series_id,post_id,next_post_id,prev_post_id)
  VALUES ('{$sid}','{$epid}',NULL,NULL)");
        ...
        if($clear_tags) {
            InSeriesInternal::wp_insert_post($post);
        }
    }
...
add_action('plugins_loaded', array('InSeriesInternal','load_l10n'));
